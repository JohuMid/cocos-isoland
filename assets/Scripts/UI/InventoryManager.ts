...
export class InventoryManager extends RenderManager {
    ...
    handleLeftBtn() {
        ...
        const isInventoryItems = DataManager.Instance.items.filter(i => i.status === ItemStatusEnum.Inventory)
        const curIndex = isInventoryItems.findIndex(i => i.type === DataManager.Instance.curItemType)
        if (curIndex > 0) {
            DataManager.Instance.isSelect = false
            DataManager.Instance.curItemType = isInventoryItems[curIndex - 1].type
        }
    }
    ...
    handleRightBtn() {
        ...
        const isInventoryItems = DataManager.Instance.items.filter(i => i.status === ItemStatusEnum.Inventory)
        const curIndex = isInventoryItems.findIndex(i => i.type === DataManager.Instance.curItemType)
        if (curIndex < isInventoryItems.length - 1) {
            DataManager.Instance.isSelect = false
            DataManager.Instance.curItemType = isInventoryItems[curIndex + 1].type
        }
    }
    ...
    changeBtnInteractable() {
        ...
        this.rightBtn.interactable = curIndex < isInventoryItems.length - 1
    }
}
